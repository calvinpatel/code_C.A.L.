...
- ⬜ **File I/O & Data Persistence** — *~5-6 lessons*
  Reading/writing text files, working with paths (`pathlib`), CSV module, JSON module, context managers revisited, encoding issues (UTF-8 vs others), handling malformed data. Bridge to everything external-world related.
...
- ⬜ **Decorators & Higher-Order Functions** — *~5-6 lessons*
  Functions as first-class citizens, closures, `*args`/`**kwargs` deep dive, writing decorators, `@property`, `functools.wraps`, real-world decorator patterns (logging, timing, auth). Bridge to FastAPI, LangChain, Flask.
